...
```bash
npx claude-cursor-coordinator init
```

One command, cross-platform. Copies all coordination files into your repo, installs Husky hooks, and runs `bun gen:rules` for you.
...
cp -r claude-cursor-coordinator/{scripts,docs,AGENTS.md,coordinator.config.json,LICENSE} your-repo/

# Windows PowerShell equivalent:
Copy-Item -Recurse claude-cursor-coordinator\.husky, claude-cursor-coordinator\.coordination, claude-cursor-coordinator\.cursor, claude-cursor-coordinator\.claude, claude-cursor-coordinator\.github your-repo\
Copy-Item claude-cursor-coordinator\.gitattributes, claude-cursor-coordinator\.gitmessage, claude-cursor-coordinator\coordinator.config.json your-repo\
Copy-Item -Recurse claude-cursor-coordinator\scripts, claude-cursor-coordinator\docs your-repo\
Copy-Item claude-cursor-coordinator\AGENTS.md, claude-cursor-coordinator\LICENSE your-repo\
...
That's it. Every commit is now trailer-signed. Both AIs run `bun status` / read `git log` (and `HANDOFF.md`) before starting new work.
...
✓ **Per-tool activity on demand.** `bun status` and `bun whodid` derive recent activity from `git log` — no tracked log file, no merge conflicts, no dirty tree.
...
coordinator.config.json                ← single source: agent identities + scope globs

.coordination/
  INDEX.md                             ← read-this-first index
  HANDOFF.md                           ← sticky notes for in-flight work
  (recent.jsonl is gitignored — activity comes from git log on demand)
...
cli/
  init.mjs                             ← npx init installer

.husky/
  pre-commit, commit-msg, pre-push     ← hooks (post-commit removed in v2)
...
bun status / bun whodid → derive per-tool activity from git log on demand
```

Both AIs run `bun status` / read `git log` (and `HANDOFF.md`) before starting new work. They see what the other one shipped recently and avoid stomping.
...
- **`coordinator.config.json`** — add agents (Codex, Gemini, Aider, etc.) and configure scope globs + priorities. The `commit-msg` hook, `trailer-guard` Action, `.gitmessage`, and `gen-rules` all read from this single file.
...
- Generated outputs: `CLAUDE.md`, `.cursorrules`, `.cursor/rules/*.mdc`, `.gitmessage`
